...
export default async function SlotsPage({ searchParams }: { searchParams: { tab?: string } }) {
...
async function SlotsTab() {
  const supabase = createClient();
  const { data: upData, error: slotErr } = await supabase
  ...
  return (
    <>
      {tableMissing ? (
        <div className="notice">予約テーブルが未作成です。Supabase で <code>0011</code> を実行してください。</div>
      ) : null}
      <p className="meta">カレンダーで追加したい場合は「予約」→「カレンダー」から空白をドラッグ／タップしてください。</p>
      <SlotForm />
      <div className="card">
...
async function TemplatesTab() {
  ...
  return (
    <>
      <p className="meta">よく使う曜日・時間帯のテンプレを保存して、期間を指定してまとめて空き枠を作れます。</p>
