...
export default async function EditAppointmentPage({ params }: { params: { id: string } }) {
  ...
  const [apptRes, patRes] = await Promise.all([
    supabase.from('appointments').select('id, patient_id, title, start_at, end_at, status, notes').eq('id', params.id).maybeSingle(),
    supabase.from('patient').select('id, name').is('deleted_at', null).order('name'),
  ]);
  if (!apptRes.data) notFound();
  const a = apptRes.data as { id: string; patient_id: string | null; title: string | null; start_at: string; end_at: string; status: string; notes: string | null };
  ...
  return (
    <>
      ...
      <div className="container">
        ...
        <AppointmentForm patients={patients} initial={initial} />
      </div>
    </>
  );
}
