...
-- =============================================================================
-- 0022_karte_visibility.sql  先生→患者の基本カルテ公開制御(既定=全公開)
-- =============================================================================
...
CREATE TRIGGER trg_karte_vis_updated BEFORE UPDATE ON karte_visibility FOR EACH ROW EXECUTE FUNCTION set_updated_at();
ALTER TABLE karte_visibility ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS karte_vis_staff ON karte_visibility;
CREATE POLICY karte_vis_staff ON karte_visibility USING (tenant_id = app_current_tenant()) WITH CHECK (tenant_id = app_current_tenant());
DROP POLICY IF EXISTS karte_vis_self ON karte_visibility;
CREATE POLICY karte_vis_self ON karte_visibility FOR SELECT USING (patient_id = app_current_patient());
